...
use crate::{GLRenderWindow, get_egl_functions};
...
unsafe fn ponev_swap_buffers() {
    unsafe { br_swap_buffers() }
}

unsafe fn ponev_make_current(window: *mut GLRenderWindow) {
    unsafe {
        br_make_current(if window.is_null() {
            None
        } else {
            Some(&mut *window)
        })
...
unsafe fn ponev_create_context(context_src: *mut GLRenderWindow) -> GLRenderWindow {
    unsafe {
        br_init_context(if context_src.is_null() {
            None
        } else {
            Some(&*context_src)
        })
...
unsafe fn ponev_swap_interval(interval: i32) {
    unsafe { br_swap_interval(interval) }
}
