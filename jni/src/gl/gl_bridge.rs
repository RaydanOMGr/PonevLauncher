...
use egli::egl::{EGLBoolean, EGLConfig, EGLDisplay, EGLint, EGL_ALPHA_SIZE, EGL_BAD_SURFACE, EGL_BLUE_SIZE, EGL_CONTEXT_CLIENT_VERSION, EGL_DEFAULT_DISPLAY, EGL_DEPTH_SIZE, EGL_GREEN_SIZE, EGL_HEIGHT, EGL_NATIVE_VISUAL_ID, EGL_NONE, EGL_NO_CONTEXT, EGL_NO_DISPLAY, EGL_NO_SURFACE, EGL_OPENGL_API, EGL_OPENGL_ES_API, EGL_RED_SIZE, EGL_RENDERABLE_TYPE, EGL_SURFACE_TYPE, EGL_TRUE, EGL_WIDTH};
...
pub unsafe fn gl_init() -> Result<(), Error> {
    ...
    *guard = Some(AtomicPtr::new(display as *mut EGLDisplay));
    ...
    if display == EGL_NO_DISPLAY {
        return Err(Error::new(ErrorKind::Other, "eglGetDisplay(EGL_DEFAULT_DISPLAY) returned EGL_NO_DISPLAY"));
    }

    if (*egl).initialize(display, 0 as *mut EGLint, 0 as *mut EGLint) != EGL_TRUE {
        let err = (*egl).get_error();
        return Err(Error::new(ErrorKind::Other, format!("eglInitialize_p() failed: {err}")))
    }
...
pub unsafe fn gl_init_context(share: Option<&GLRenderWindow>) -> Result<GLRenderWindow, Box<dyn std::error::Error>> {
    ...
    let egl_attributes = [ EGL_BLUE_SIZE, 8, EGL_GREEN_SIZE, 8, EGL_RED_SIZE, 8, EGL_ALPHA_SIZE, 8, EGL_DEPTH_SIZE, 24, EGL_SURFACE_TYPE, EGL_WINDOW_BIT|EGL_PBUFFER_BIT, EGL_RENDERABLE_TYPE, EGL_OPENGL_ES2_BIT, EGL_NONE ].as_ptr();
    let num_configs = 0 as *mut EGLint;

    if (*egl).choose_config(*display, egl_attributes, ptr::null_mut(), 0, num_configs) != EGL_TRUE {
        let err = (*egl).get_error();
        return Err(Error::new(ErrorKind::Other, format!("eglChooseConfig() failed: {err}")).into());
    }
    if num_configs == 0 as *mut EGLint {
        return Err(Error::new(ErrorKind::Other, "eglChooseConfig() found no matching config").into());
    }

    (*egl).choose_config(*display, egl_attributes, &mut bundle.config as *mut EGLConfig, 1, num_configs);
    (*egl).get_config_attrib(*display, bundle.config, EGL_NATIVE_VISUAL_ID, &mut bundle.format as *mut EGLint);

    let bind_result: EGLBoolean;
    if env::var("MOJO_RENDERER")? == "opengles3_desktopgl" {
        println!("EGLBridge: Binding to desktop OpenGL\n");
        bind_result = (*egl).bind_api(EGL_OPENGL_API);
    } else {
        println!("EGLBridge: Binding to OpenGL ES\n");
        bind_result = (*egl).bind_api(EGL_OPENGL_ES_API);
    }
    ...
    bundle.context = (*egl).create_context(
        *display,
        bundle.config,
        if let Some(value) = share { (&value).context } else { EGL_NO_CONTEXT },
        egl_context_attributes
    );

    if bundle.context == EGL_NO_CONTEXT {
        let err = (*egl).get_error();
        return Err(Error::new(ErrorKind::Other, format!("eglCreateContext() finished with error: {err}")).into())
    }
...
pub unsafe fn gl_swap_surface(bundle: &mut GLRenderWindow) {
    ...
    if !(&bundle).surface.is_null() {
        (*egl).destroy_surface(*display, (&bundle).surface);
    }
    if let Some(new_surface) = (&bundle).new_native_surface {
        ...
        bundle.surface = (*egl).create_window_surface(*display, bundle.config, bundle.native_surface.unwrap(), ptr::null_mut());
    }else{
        ...
        bundle.surface = (*egl).create_pbuffer_surface(*display, bundle.config, pbuffer_attrs);
    }
}
...
pub unsafe fn gl_make_current(bundle: Option<&mut GLRenderWindow>) {
    ...
    if (&bundle).is_none() {
        if (*egl).make_current(*display, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT) == EGL_TRUE {
    ...
    if (*environ).main_window_bundle.take().is_none() {
        ...
        (*environ).main_window_bundle.set(Some(ptr as *mut BasicRenderWindow));
        println!("Main window bundle is now {:?}", (*environ).main_window_bundle.get_mut());
    ...
    println!("Making current, surface={:?}, nativeSurface={:?} newNativeSurface={:?}", unwrapped.surface, unwrapped.native_surface, unwrapped.new_native_surface);
    if unwrapped.surface.is_null() {
        gl_swap_surface(&mut unwrapped);
    }
    if (*egl).make_current(*display, unwrapped.surface, unwrapped.surface, unwrapped.context) == EGL_TRUE {
        CURRENT_BUNDLE.with(|bundle| *bundle.lock().unwrap() = None);
    } else {
...
pub unsafe fn gl_swap_buffers() {
    ...
    &CURRENT_BUNDLE.with(|b| {
        ...
        if (*bundle_guard).state == STATE_RENDERER_NEW_WINDOW {
            ...
            (*egl).make_current(*display, (*bundle_guard).surface, (*bundle_guard).surface, (*bundle_guard).context);
            (*bundle_guard).state = STATE_RENDERER_ALIVE;
        }
        if !(*bundle_guard).surface.is_null() && (*egl).swap_buffers(*display, (*bundle_guard).surface) != EGL_TRUE && (*egl).get_error() == EGL_BAD_SURFACE {
...
pub fn native_init_gl4es_internals(mut env: JNIEnv, _class: JClass, function_provider: JObject) {
    ...
    let func_provider_class = env.get_object_class(&function_provider).unwrap();
    ...
    if let Some(set_getmainfbsize_fn) = set_getmainfbsize {
        ...
        unsafe { set_getmainfbsize_fn(gl4esi_get_display_dimensions); }
    }
}
